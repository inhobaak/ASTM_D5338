const ASTM_CHANNELS = {
    ...
    'pico-ch-02': { label: 'Cellulose', role: 'Positive control', colorVar: '--cellulose' },
...
const alertLastFired = {}; // key: `${rule.id}|${channel_id}`
...
let channelColors = {};
let alertRules = [];
let channelData = {}; // { channel_id: { co2: [], temp: [], humidity: [] } }
...
function buildChannelCard(ch) {
    ...
    div.innerHTML = `
        ...
        <div class="metric-row">
            <span class="metric-label">CO2 ppm</span>
...
function rebuildChartDatasets() {
    ['co2', 'temp', 'humidity'].forEach(metric => {
        const chart = metric === 'co2' ? charts.co2
                    ...
                    : charts.humidity;
        chart.data.datasets = Object.entries(channelData).map(([chId, d]) => ({
...
function checkAlertThresholds(r) {
    alertRules.forEach(rule => {
        ...
        if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('ASTM D5338 CO2 Alert', { body: msg });
        }
    });
}
...
function addAlertLogEntry(msg) {
    ...
    // Cap log at ALERT_LOG_MAX entries to prevent unbounded DOM growth
